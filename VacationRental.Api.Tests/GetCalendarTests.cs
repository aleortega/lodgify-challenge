...
namespace VacationRental.Api.Tests
{
    ...
    public class GetCalendarTests
    {
        ...
        [Fact]
        public async Task GivenCompleteRequest_WhenGetCalendarWithOneDayOfPreparationTime_ThenAGetReturnsTheCalculatedCalendarWithPreparationTimes()
        {
            var postRentalRequest = new RentalBindingModel
            {
                Units = 2,
            ...
            using (var getCalendarResponse = await _client.GetAsync($"/api/v1/calendar?rentalId={postRentalResult.Id}&start=2000-01-01&nights=5"))
            {
                ...
                Assert.Equal(new DateTime(2000, 01, 02), getCalendarResult.Dates[1].Date);
                Assert.Single(getCalendarResult.Dates[1].Bookings);
                Assert.Contains(getCalendarResult.Dates[1].Bookings, x => x.Id == postBooking1Result.Id && x.Unit == 2);
                Assert.Empty(getCalendarResult.Dates[1].PreparationTimes);

                Assert.Equal(new DateTime(2000, 01, 03), getCalendarResult.Dates[2].Date);
                Assert.Equal(2, getCalendarResult.Dates[2].Bookings.Count);
                Assert.Contains(getCalendarResult.Dates[2].Bookings, x => x.Id == postBooking1Result.Id && x.Unit == 2);
                Assert.Contains(getCalendarResult.Dates[2].Bookings, x => x.Id == postBooking2Result.Id && x.Unit == 1);
                Assert.Empty(getCalendarResult.Dates[2].PreparationTimes);

                Assert.Equal(new DateTime(2000, 01, 04), getCalendarResult.Dates[3].Date);
                Assert.Single(getCalendarResult.Dates[3].Bookings);
                Assert.Contains(getCalendarResult.Dates[3].Bookings, x => x.Id == postBooking2Result.Id && x.Unit == 1);
        ...
        [Fact]
        public async Task GivenCompleteRequest_WhenGetCalendarWithTwoDaysOfPreparationTime_ThenAGetReturnsTheCalculatedCalendarWithPreparationTimes()
        {
            var postRentalRequest = new RentalBindingModel
            {
                Units = 2,
